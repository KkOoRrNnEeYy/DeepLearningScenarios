...
class ODEnet:
    def __init__(self, shape, eta1, eta2, LR):
        ...
        self.iters = 0
        self.init_step = [-0.01 for i in range(self.S+1)]
        self.min_step = [0.000001 for i in range(self.S+1)]
    ...
    def trained(self):
        z1 = np.array([*self.ZA[:self.S//2][0], *self.ZA[:self.S//2][-1]])
...
for t in range(5, 100):
    ...
    file = str(n)+'N.T=' + str(np.round(t/10, 2)) + ',e1='+str(E1)+',lr='+str(LR)+'.csv'
